// Fabryka providerów odczytu umowy. Wybór: opcja > EXTRACTOR_PROVIDER > "demo".

import { validateUmowa } from "./schema.mjs";
...
export async function extractUmowa(input, opts = {}) {
  ...
  const { ok, errors, value, braki } = validateUmowa(raw);
  return { provider: name, raw, value, ok, errors, braki };
}
